...
import {
  ApprovalStatusCheck,
  CheckExistingCheckInRequest,
  GenerateCheckInOTP,
  GetPreviewData,
  GuestSubmitHotelChekInRequest,
  HotelCheckInStep1,
  SendGuestAadhaarOTP,
  VerifyGuestAadhaarOTP,
  VerifyGuestPassport,
  VerifyGuestXipperID,
...
const VerifyContent = ({setHeading, toggleModal, setIsCheckIn, hotelData}) => {
  ...
  const getCheckInStatus = async () => {
    try {
      ...
      setCheckInData(res);
      if (
        res.existingPendingRequests.length > 0 ||
        res.submittedRequests.length > 0
      ) {
        setStep(6);
      }
    } catch (e) {
      ...
      switch (step) {
        case 1:
          ...
          const resp = await HotelCheckInStep1(
            hotelData?.XipperID,
            selectedProfile.type === 'company' ? selectedProfile.XipperID : '',
            formData,
          );
          if (resp.status === 200) {
            if (formData.totalGuests === 1) {
              data = {
                XipperId: selectedProfile.XipperID,
                name: selectedProfile.name,
                verificationType: 'Xipper ID',
                verificationNumber: selectedProfile.XipperID,
              };
            ...
            setFormData({
              ...formData,
              requestId: resp.data.requestId || resp.requestId,
            });
          }
          break;
        ...
        case 2:
          if (selectedVerification === 'Aadhaar') {
            ...
            if (res.status !== 200) {
              ...
              setFormData({
                ...formData,
                aadhaarOtpRefId: res.data.data.ref_id,
                [`aadhaar${currentGuest}`]: verificationNumber,
                xipperId: '',
              });
              setStep(3);
            }
          } else if (selectedVerification === 'Passport') {
            const verificationResponse = await VerifyGuestPassport(
              passportData,
              formData,
            );
            ...
            if (verificationResponse.status !== 200) {
              ...
              data = {
                ...
                verificationNumber: passportData.file_number,
              };
              setGuest(prevGuests => [...prevGuests, data]);
            ...
            if (verificationResponse.status !== 200) {
              ...
              data = {
                ...
                verificationType: selectedVerification,
                verificationNumber: formData.xipperId,
                name: '(Unverified)',
              };
              setGuest(prevGuests => [...prevGuests, data]);
              if (currentGuest < formData.totalGuests) {
          ...
          break;
        case 3:
          const aadhaarResponse = await VerifyGuestAadhaarOTP(
            otp,
            formData,
            formData?.[`aadhaar${currentGuest}`],
          );
        ...
        case 4:
          const res = await GuestSubmitHotelChekInRequest(
            formData,
            previewData,
          );
          console.log(res);
    ...
    if (data.tempUserCheckInInfo.length === data.totalGuests) {
      ...
      data.tempUserCheckInInfo.map(item => {
        guestData.push({
          ...
          name: item.user ? item.user.fullName : '(Unverified)',
          verificationType: 'Xipper ID',
      ...
      data.tempUserCheckInInfo.forEach((item, index) => {
        if (
          data.tempUserCheckInInfo.length === 1 &&
          item.tempNonXipperUserInfo === null
        ) {
          guestData.push({
            XipperId: item.XipperId,
          ...
          if (
            item.XipperId === null &&
            item.tempNonXipperUserInfo &&
            Object.keys(item.tempNonXipperUserInfo).length > 0
          ) {
            guestData.push({
              XipperId: '',
              name: item.tempNonXipperUserInfo.name,
              verificationType:
                item.tempNonXipperUserInfo.aadhaarNumber === null
                  ? 'Passport File Number'
                  : 'Aadhaar Number',
              verificationNumber:
                item.tempNonXipperUserInfo.aadhaarNumber === null
                  ? item.tempNonXipperUserInfo.PassportFileNumber
                  : item.tempNonXipperUserInfo.aadhaarNumber,
            });
          }
  ...
  return (
    <>
      {step < 5 && (
        <Pressable className="mt-[-50px] mb-10 w-10" onPress={handleBack}>
          <BackArrowIcon />
        </Pressable>
      )}
        ...
        <>
          ...
          <View className="flex-row items-center mt-2">
            ...
            <Text className="font-pmedium text-md text-black pr-5 ml-1">
              {`${hotelData?.locality || hotelData?.address?.locality || ''}, ${
                hotelData?.city || hotelData?.address?.city || ''
              }, ${hotelData?.state || hotelData?.address?.state || ''} - ${
                hotelData?.pincode || hotelData?.address?.pincode || ''
              }`}
            </Text>
          </View>
        </>
      ...
      {step === 1 && (
        <View>
              ...
              <View className="w-full flex gap-2 flex-row justify-between mt-4">
                <View
                  ...
                  <TextInput
                    value={
                      formData.checkIn
                        ? formatDate(new Date(formData.checkIn))
                        : ''
                    }
          ...
          <View className="w-full flex gap-2 flex-row justify-between mt-2">
            ...
            <View className="w-[48%] flex flex-row items-center justify-between border border-gray-200 rounded-md p-1">
              ...
              <Picker
                ...
                itemStyle={Platform.OS === 'ios' ? styles.pickerItem : ''}>
                {Array.from(
                  {length: formData.totalGuests || 10},
                  (_, i) => i + 1,
                ).map(room => (
          ...
          <Pressable
            className={`px-2 py-2 my-5 mx-6 items-center rounded-md ${
      ...
      {step === 2 && (
        <View>
          <View className="flex-row items-center justify-between  mt-3">
            ...
            <Pressable onPress={() => setShowRadioOptions(!showRadioOptions)}>
              <DropDownArrow />
            </Pressable>
          </View>
          ...
          <Pressable
            className={`px-2 py-2 mx-6 items-center rounded-md ${
              selectedProfile.type === 'user' ? 'bg-user' : 'bg-company'
            }`}
            onPress={handleSkip}>
            <Text className="font-pmedium text-center text-lg text-white">
      ...
      {step === 4 && (
        <View>
          {!loading &&
            previewData?.tempUserCheckInInfo?.map((g, index) => (
              <View key={index} className="mb-3">
                <Text className="font-psemibold text-base text-black mb-2">
                  Guest {index + 1}{' '}
                  {g?.role === 'Primary Guest' && '(Primary Guest)'}
                </Text>
                {g?.unverified ? (
                  ...
                  <>
                    ...
                    {(g?.role === 'Primary Guest' || g.XipperId) && (
                      <Text className="font-pmedium text-md text-black mb-1">
                        {`${
                          g?.role === 'Primary Guest' || g.XipperId
                            ? `Xipper ID : ${g.XipperId}`
                            : ''
                        }`}
                      </Text>
                    )}
                    {g.tempNonXipperUserInfo &&
                      g.tempNonXipperUserInfo.aadhaarNumber && (
                        <Text className="font-pmedium text-md text-black mb-1">
          ...
          {!loading &&
            previewData?.totalGuests !==
              previewData?.tempUserCheckInInfo?.length &&
            Array(
              previewData?.totalGuests -
                previewData?.tempUserCheckInInfo?.length,
            )
        ...
        (isConfirm ? (
          <>
            {appCode.length === 6 ? (
                ...
                <Pressable onPress={handleGenerateCode}>
                  <Text
                    className={`text-center  ${
                      selectedProfile.type === 'user' ? 'bg-user' : 'bg-company'
                    } py-2 mx-5 rounded-md text-white font-psemibold mt-3`}>
                    Re-Generate Code
                  </Text>
                </Pressable>
              </>
              ...
              <>
                <Text
                  className={`font-pmedium text-center text-sm mt-[10px] mb-3 ${
                    selectedProfile.type === 'user'
                      ? 'text-user'
                      : 'text-company'
                  }`}>
                ...
                <Text className="font-pmedium text-center text-sm text-black mb-3">
                  If you are at the property, kindly generate the app code and
                  share it with the receptionist.
                </Text>
                <Pressable
                  disabled={
                    formatDate(new Date(formData.checkIn)) !==
                  ...
                  <Text
                    className={`text-center py-2 mx-5 rounded-md text-white font-psemibold mt-3 ${
                      selectedProfile.type === 'user' ? 'bg-user' : 'bg-company'
                    }`}>
                    Genrate Code
                  </Text>
      ...
      {step === 6 && (
        <View>
          <ScrollView
            ...
            <View className="w-full flex gap-2 flex-col justify-between mt-4">
              {checkInData.existingPendingRequests.map((i, ind) => (
                <Pressable
                  key={ind}
                  onPress={() => handleExistingPendingRequest(i)}
                  ...
                  <Text className="p-2 text-black">PNR : {i.bookingId}</Text>
                </Pressable>
              ))}
              {checkInData.submittedRequests.map((i, ind) => (
                <Pressable
                  key={ind}
                  onPress={() => handleSubmittedRequest(i)}
          ...
          <Pressable
            onPress={() => setStep(1)}
            className={`w-full mt-4 flex flex-row items-center justify-center mb-2 p-1 border border-gray-200 rounded-md ${
              selectedProfile.type === 'user' ? 'bg-user' : 'bg-company'
            }`}>
