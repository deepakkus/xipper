...
import { AddLaundryItemsToCart, AddSpaItemsToCart, GetLaundryCart, GetSpaServicesCart, RemoveLaundryItemsToCart, RemoveSpaItemsFromCart } from '../services/servicesService';
import CircularLoader from './CircularLoader';
import { useDispatch, useSelector } from 'react-redux';
import { setLaundryCart, setLaundryCartId, setSpaCartId, setSpaServiceCart } from '../redux/servicesRedux';

export default function SpaCart() {
    const navigation = useNavigation();
    const dispatch = useDispatch();
    const route = useRoute();
    const { selectedProfile, selectedProperty } = useSelector(state => state.account);
    const { spaCartId, spaServiceCart } = useSelector(state => state.services);
    ...
    const getCart = async () => {
        try {
            setLoading(true);
            const res = await GetSpaServicesCart(spaCartId);
            dispatch(setSpaServiceCart(res.data));
    ...
    const handleCart = async (item, type) => {
        try {
            let res;
            if (type === "add") {
                res = await AddSpaItemsToCart(selectedProperty, item.itemid);
            }
            if (type === "remove") {
                res = await RemoveSpaItemsFromCart(selectedProperty, item.itemid);
            }
            console.log(res)
            await getCart();
            if (res.data.message === "Cart not found" || res.data.cart === "Cart Empty") {
                dispatch(setSpaServiceCart({}));
            }
            return res;
        } catch (e) {
            console.log(e);
    ...
    return (
        <ScrollView
            ...
            {spaServiceCart?.cartResponse?.length > 0 && (
                spaServiceCart?.cartResponse?.map((item, index) => (
            ...
            {spaServiceCart?.bill?.priceBreakup?.length > 0 && (
                <View className="p-4 bg-white rounded-lg shadow mt-4">
                    <Text className="text-lg font-semibold mb-2 text-black">Price Breakup</Text>
                    {spaServiceCart?.bill?.priceBreakup?.map((item, index) => (
                    ...
                    <View className="mt-4 space-y-2">
                        ...
                        {spaServiceCart.bill.totalPriceBeforeTax && <View className="flex-row justify-between">
                            <Text className="text-sm text-black">Total Before Tax</Text>
                            <Text className="text-black">₹ {spaServiceCart.bill.totalPriceBeforeTax}</Text>
                        </View>}
                        {spaServiceCart.bill.totalTax && <View className="flex-row justify-between">
                            <Text className="text-sm text-black">Total Taxes</Text>
                            <Text className="text-black">₹ {spaServiceCart.bill.totalTax}</Text>
                        </View>}
                        {/* <View className="flex-row justify-between">
                    ...
                    <View className="flex-row justify-between mt-4 pt-2 border-t border-gray-200">
                        <Text className={`text-lg font-semibold text-${selectedProfile.type}`}>Amount To Pay</Text>
                        <Text className={`text-lg font-semibold text-${selectedProfile.type}`}>
                            ₹ {spaServiceCart.bill.grandTotal}
                        </Text>
                    </View>
                </View>
            )}
