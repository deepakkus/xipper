...
import { GetFamily, AddFamilyMem, DeleteFamily } from '../../services/profileService';
          ...
          <View className="flex-row justify-between mt-4">
            {data.length > 0 ? (
              data.map((member, index) => (
                <View 
                  key={index} 
                  className="flex-1 h-28 bg-white rounded-lg shadow mx-2 p-2"
                >
                  <View className="flex-row justify-between">
                    <Text className="text-sm text-gray-500">{member.relationship}</Text>
                    <Pressable
                  ...
                  <View className="flex-row items-center mt-2">
                    ...
                    />
                    <View>
                      <Text className="text-sm font-semibold  text-gray-500 mr-2">{member.fullName}</Text>
                      <Text className="text-xs text-gray-400">{member.xipperID}</Text>
                    </View>
                  </View>
                </View>
              ))
            ) : (
              <Text className="text-center mt-4 text-gray-500">No family members found</Text>
            )}
          </View>
        </View>
      </ScrollView>
